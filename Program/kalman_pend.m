...
for i=2:timepts
    ...
    xkalman(:,i)=A*xkalmanf(:,i-1) + B*u;
    x_error(:,i)=x(:,i)-xkalman(:,i);
    Pk=A*Pk1*A';
    z=H*x(:,i)+0.01*randn;
    yhat=z-H*xkalman(:,i);
    S=H*Pk*H' + 0.01;
    K_g=Pk*H'*(S^-1);
    xkalmanf(:,i)=xkalman(:,i)+K_g*yhat;
    Pk1=(I-K_g*H)*Pk;
    x_error1(:,i)=x(:,i)-xkalmanf(:,i);
end

final_state = x;
kalman_state = xkalmanf;
%% Graphical Plots
...
plot(t,final_state(1,:)*180/pi,'linewidth',2,'Color',[0,0,0]);
hold on;
plot(t,kalman_state(1,:)*180/pi,'linewidth',2,'Color',[0,0.7,0.9]);
title('Angle (degrees)')
% axis([0 20 -100 400])
subplot(222)
...
% hold on
plot(t,kalman_state(4,:),'linewidth',2);
...
% figure('Color','white');
% 
% h2(1) = plot([kalman_state(3,1),kalman_state(3,1)+x(1);],[0,y(1)],'r.-','MarkerSize',20,'LineWidth',3);
% hold on;
% h2(2) = plot([kalman_state(3,1),cart_len+kalman_state(3,1)],[0,0],'k-','LineWidth',3);
% % axis([-1 2 -0.5 0.5]);
% ht = title(sprintf('Time: %0.3f sec',t(1)));
% 
% pos = get(gcf, 'Position');
...
% for id = 1:1:timepts
%     set(h1, 'XData',t(id),'YData',ang(id))
%     set(h2(1), 'XData',[kalman_state(3,id),kalman_state(3,id)+x(id)], 'YData', [0,y(id)]);
%     set(h2(2), 'XData',[kalman_state(3,id),kalman_state(3,id)+cart_len], 'YData', [0,0]);
%     set(ht, 'String', sprintf('Time: %0.3f sec',t(id)));
%     f = getframe(gcf);
%     
% %     if id == 1
