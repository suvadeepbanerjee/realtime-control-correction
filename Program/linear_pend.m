...
duration = 5;       % Total Time of running
...
theta_0 = -30*pi/180;      % Initial Angle of Pendulum to start with (range -0.15*pi to 0.15*pi)
x(1,1) = theta_0;
...
% K = 0.95*place(sysd.a,sysd.b,[z1;z2;z3;z4]);      % Faulty Gain
...
% for i = 1:floor(timepts/2)
% 
% figure(4)
% plot(xcart1(round(length(xcart1)/2))+x(3,2*i)+xline*sin(x(1,2*i)),yline*cos(x(1,2*i)),'r','linewidth',3);
% hold on;
% plot(round(length(xcart1)/2)+x(3,2*i)+xline(end)*sin(x(1,2*i)),yline(end)*cos(x(1,2*i)),'ro','linewidth',3);
% plot(xcart1+x(3,2*i),zeros(size(xcart1)),'b','linewidth',3);
% hold off;
% axis([-2 0.5 -0.5 0.5]);
% 
% end
